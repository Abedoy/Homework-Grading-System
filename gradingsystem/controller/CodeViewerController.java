    ...
    void handledownloadButtonAction(ActionEvent event)    {
        ...
        DirectoryChooser directoryChooser = new DirectoryChooser();
             ...
             // Replace this section with getSourceFile from database********
              File srcFile = new File(pSFM.getSourceFileByID(2).getFileName());
              FileWriter fileWriter;
			try {
				fileWriter = new FileWriter(file);
			
              fileWriter.write(fileToText((pSFM.getSourceFileByID(2)).getSourceFile()));
              fileWriter.close();
			} catch (IOException e) {
        ...
        try {
            ...
            byte[] data = Files.readAllBytes((pSFM.getSourceFileByID(2)).getSourceFile().toPath());
            out.write(data);
            out.close();
           // fileWriter.close();
